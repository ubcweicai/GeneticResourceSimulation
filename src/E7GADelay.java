		...
		for(int simu_time=0; simu_time<SimulationTimes; simu_time++)
		{
			...
			for(MobileDeviceQuantity=MobileDeviceQuantityStart; MobileDeviceQuantity<=maxMobileDeviceQuantity; MobileDeviceQuantity=MobileDeviceQuantity+MobileDeviceQuantityStep)
			{
				...
				***/
					...
					System.out.println("terminal value: "+allCloudValidResult.averageDeviceNetworkLatency+". It is "+allCloudValidResult.valid);
					write_Terminal.add(String.format("%.2f",allCloudValidResult.averageDeviceNetworkLatency));
				...
				***/
				...
				System.out.println("=========================Single Device Delay Minimization Mode========================");
				...
				if((!pure_optimal_solution.isEmpty())&&(!mere_optimal_solution.isEmpty()))
				{
					...
					if((gat==1)||(gat==100)||(gat==200)||(gat==300))
					{
						...
						if(!GASolutions.isEmpty())
						{
							...
							if(gavr.valid)
							{
								...
								System.out.println(gat+" ga at [0] min value: "+gavr.averageDeviceNetworkLatency);

								write_GA_single.add(String.format("%.2f",gavr.averageDeviceNetworkLatency));
								////showState(gavr);
								//showDetailedState(gavr);
							}else
							{
								minGAValidResult = minValidResult2;
								write_GA_single.add(String.valueOf(INFINITE));
				...
				if(!GASolutions.isEmpty())
				{
					...
					if(gavr.valid)
					{
						//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
						minGAValidResult = gavr;
						float minValue=gavr.averageDeviceNetworkLatency;
						...
						{	
							...
							if(ga_endless_vr.valid)
							{
								...
								minEndlessValue=ga_endless_vr.averageDeviceNetworkLatency;
							}
					...
					{
						minGAValidResult = minValidResult2;
						write_Boundary.add(String.valueOf(INFINITE));
			...
			{
				saveOneSimulationResultToFile(write_DeviceQuantity, GA_ITERATIN_START, GA_ITERATIN_STEP, GA_ITERATIN_END, write_Terminal, write_Boundary, write_GA, "genetic_result_delay");
			}
