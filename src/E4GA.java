...
public class E4GA {
	...
	public static void main (String args[])
	{
		...
		//Latency Factor
		float Cloud_Latency_Factor=0.00004f;
		float Device_Latency_Factor=0.00035f;
		float Bandwidth_Latency_Factor=0.004f;
		...
		int AvailableBandwidth=100;
		int randomAvailableBandwidth_range = 400;
		int AvailableResources=100;
		int randomAvailableResources_range = 100;
		int TolerateLatency=120;
		int MobileDeviceQuantity=10;
		...
		for(MobileDeviceQuantity=MobileDeviceQuantityStart; MobileDeviceQuantity<=maxMobileDeviceQuantity; MobileDeviceQuantity++)
		{
			...
			for(int simu_time=0; simu_time<SimulationTimes; simu_time++)
			{
					...
					if(allCloudValidResult.valid==true)
					{
						...
						write_Terminal=String.format("%.2f",allCloudValidResult.averageDeviceNetworkLatency);
						////showState(allCloudValidResult);
						//showDetailedState(allCloudValidResult);
					}else
					{
						//terminal.add(INFINITE);
						write_Terminal=String.valueOf(INFINITE);
					...
					if(!GASolutions.isEmpty())
					{
						...
						if(gavr.valid)
						{
							...
							//GA1.add(String.format("%.2f",gavr.averageDeviceNetworkLatency));
							write_GA1=String.format("%.2f",gavr.averageDeviceNetworkLatency);
							////showState(gavr);
						...
						}else
						{
							...
							//GA1.add(INFINITE);
							write_GA1=String.valueOf(INFINITE);
					...
					System.out.println("=========================GA2 Mode========================");
					
					//Parameter 
					GA_ITERATION=10;
					
					minAverageDeviceLatency=INFINITE;
					minValidResult2=null;
					minGAValidResult=null;
					
					gap = new GAProcessor(ComponentQuantity, MobileDeviceQuantity,factors);
					GASolutions = gap.GASolutions(GA_SPACE_SIZE);
					...
					{	
						...
						//System.out.println("----- Population of grown genes: "+GASolutions.size() +" -----");
						GASolutions = gap.sortGASolutions(GASolutions, g, cs, md, minAverageDeviceLatency, minValidResult2, "AverageDeviceLatency" );
					...
					if(!GASolutions.isEmpty())
					{
						...
						if(gavr.valid)
						{
							//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
							minGAValidResult = gavr;
							//GA2.add(String.format("%.2f",gavr.averageDeviceNetworkLatency));
							write_GA2=String.format("%.2f",gavr.averageDeviceNetworkLatency);
							////showState(gavr);
							//showDetailedState(gavr);
						}else
						{
							minGAValidResult = minValidResult2;
							//GA2.add(INFINITE);
							write_GA2=String.valueOf(INFINITE);
							System.out.println("No valid gene was found!");
						}
					}else
					{
					...
					if(!GASolutions.isEmpty())
					{
						...
						if(gavr.valid)
						{
							//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
							minGAValidResult = gavr;
							//GA3.add(String.format("%.2f",gavr.averageDeviceNetworkLatency));
							write_GA3=String.format("%.2f",gavr.averageDeviceNetworkLatency);
							////showState(gavr);
							//showDetailedState(gavr);
						}else
						{
							minGAValidResult = minValidResult2;
							//GA3.add(INFINITE);
							write_GA3=String.valueOf(INFINITE);
							System.out.println("No valid gene was found!");
						}
					}else
					{
					...
					System.out.print(write_Terminal+",");
					System.out.print(write_Delay+",");
					System.out.print(write_GA1+",");
					System.out.print(write_GA2+",");
					System.out.print(write_GA3+",");
					System.out.print(write_DeviceQuantity+"\n");
					
					saveSingleToFile(write_Terminal, write_Delay, write_GA1, write_GA2, write_GA3, write_DeviceQuantity, "genetic_result");
			}
		}
	...
	public static void saveSingleToFile(String terminal, String delay, String GA1, String GA2, String GA3, int deviceQuantity, String filename)
	{
		...
		try{
			...
			dos.writeBytes(terminal+",");
			dos.writeBytes(delay+",");
			dos.writeBytes(GA1+",");
			dos.writeBytes(GA2+",");
			dos.writeBytes(GA3+",");
			dos.writeBytes(deviceQuantity+"\r");
