...
//		int ADDITIONAL_GA_ITERATION=200;
		...
		for(int simu_time=0; simu_time<SimulationTimes; simu_time++)
		{
			...
			for(MobileDeviceQuantity=MobileDeviceQuantityStart; MobileDeviceQuantity<=maxMobileDeviceQuantity; MobileDeviceQuantity=MobileDeviceQuantity+MobileDeviceQuantityStep)
			{
				...
				System.out.println("=========================Cloud Optimization=========================");		
				...
				***/
					...
					//System.out.println("terminal value: "+allCloudValidResult.cloudConsumption+". It is "+allCloudValidResult.valid);
					write_Terminal.add(String.format("%.2f",allCloudValidResult.cloudConsumption));
				...
				***/
						...
						else
						{
							...
							DataInputStream dis=new DataInputStream(fis);
						  	...
						  	while(!(aSolution = readGeneFromFile(ComponentQuantity, SingleMobileDeviceQuantity,dis)).isEmpty()){
						  		...
						  		if((avr.valid)&&(avr.cloudConsumption<minSingleAverageDeviceLatency))
								{
						  			minSingleAverageDeviceLatency=avr.cloudConsumption;
				...
				if(mere_optimal_solution.get(0).toString().length() == pure_optimal_solution.toString().length())
				{
					...
					if(lgvr.valid)
					{
						...
						if(lgvr.cloudConsumption>allCloudValidResult.cloudConsumption)
						{
							minLGValue=allCloudValidResult.cloudConsumption;
						}else
						{
							minLGValue=lgvr.cloudConsumption;
						}
				...
				{	
					...
					GASolutions = gap.sortGASolutions(GASolutions, g, cs, md, minAverageDeviceLatency2, minValidResult2, "CloudConsumption" );
...
//					if((gat==1)||(gat==100)||(gat==200)||(gat==300))
					if((gat==100)||(gat==200)||(gat==300))
					{
						...
						if(!GASolutions.isEmpty())
						{
							...
							if(gavr.valid)
							{
								...
								if(gavr.cloudConsumption>minLGValue)
								{
									write_GA_single.add(String.format("%.2f",minLGValue));
								}else
								{
									write_GA_single.add(String.format("%.2f",gavr.cloudConsumption));
								}
								////showState(gavr);
								//showDetailedState(gavr);
							}else
						...
						}else
						{
							minGAValidResult = minValidResult2;
							//GA1.add(INFINITE);
							write_GA_single.add(String.format("%.2f",allCloudValidResult.cloudConsumption));
				...
				if(!GASolutions.isEmpty())
				{
					...
					if(gavr.valid)
					{
						//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
						minGAValidResult = gavr;
						float minValue=gavr.cloudConsumption;
						...
						{	
							...
							if(ga_endless_vr.valid)
							{
								...
								minEndlessValue=ga_endless_vr.cloudConsumption;
							}
					...
					{
						minGAValidResult = minValidResult2;
						write_Boundary.add(String.format("%.2f",allCloudValidResult.cloudConsumption));
			...
			{
				saveOneSimulationResultToFile(write_DeviceQuantity, GA_ITERATIN_START, GA_ITERATIN_STEP, GA_ITERATIN_END, write_Terminal, write_Boundary, write_GA, "genetic_result_cloud");
			}
	...
	public static void readFromFile(int n, String filename)
	{
		try{
	        ...
	        {
	            FileInputStream fis=new FileInputStream(f);//￥￥Ω??‘”?fμ???o??‰????
	            DataInputStream dis=new DataInputStream(fis);
