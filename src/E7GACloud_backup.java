	...
	public static void main (String args[])
	{
		...
		for(int simu_time=0; simu_time<SimulationTimes; simu_time++)
		{
			...
			{		
				...
				ValidResult minValidResult_CloudConsumption=null;
				...
				if(allCloudValidResult.valid==true)
				{
					...
					write_Terminal=String.format("%.2f",allCloudValidResult.cloudConsumption);
				...
				float minCloudConsumption2=INFINITE;
					...
					if((gat>0)&&(gat%GA_ITERATIN_STEP==0))
					{
						...
						if(!GASolutions.isEmpty())
						{
							...
							if(gavr.valid)
							{
								...
								write_GA.add(String.format("%.2f",gavr.cloudConsumption));
								////showState(gavr);
				...
				int ADDITIONAL_GA_ITERATION;
				if(MobileDeviceQuantity!=100)
				{
					ADDITIONAL_GA_ITERATION=100*MobileDeviceQuantity;
				}else
				{
					ADDITIONAL_GA_ITERATION=20000;
				}
				...
				{	
					...
					if(!GASolutions.isEmpty())
					{
						...
						if(gavr.valid)
						{
							//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
							minGAValidResult = gavr;
							write_Boundary=String.format("%.2f",gavr.cloudConsumption);
							////showState(gavr);
							//showDetailedState(gavr);
						}else
						{
				...
				saveSingleToFile(write_DeviceQuantity, GA_ITERATIN_START, GA_ITERATIN_STEP, GA_ITERATIN_END, write_Terminal, write_Boundary, write_GA, "genetic_result_cloud");
