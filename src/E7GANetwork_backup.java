...
public class E7GANetwork_backup {
		...
		for(int simu_time=0; simu_time<SimulationTimes; simu_time++)
		{
			...
			{		
				...
				for(int i=0;i<MobileDeviceQuantity;i++)
				{
					...
					int randomAvailableBandwidth=(int)(Math.random()*180);
				...
				float minNetworkConsumption=INFINITE;
				ValidResult minValidResult_NetworkConsumption=null;
				...
				if(allCloudValidResult.valid==true)
				{
					...
					write_Terminal=String.format("%.2f",allCloudValidResult.averageDeviceBandwidth);
				...
				float minNetworkConsumption2=INFINITE;
				...
				GASolutions.set(0, cloud_solution);		
				...
				{	
					...
					GASolutions = gap.sortGASolutions(GASolutions, g, cs, md, minNetworkConsumption2, minValidResult2, "AverageDeviceBandwith" );
					...
					if((gat>0)&&(gat%GA_ITERATIN_STEP==0))
					{
						...
						if(!GASolutions.isEmpty())
						{
							...
							if(gavr.valid)
							{
								...
								write_GA.add(String.format("%.2f",gavr.averageDeviceBandwidth));
								////showState(gavr);
				...
				int ADDITIONAL_GA_ITERATION=100*MobileDeviceQuantity;
				write_Boundary="";
				...
				{	
					...
					if(!GASolutions.isEmpty())
					{
						...
						if(gavr.valid)
						{
							//System.out.println("Final cloud consumption: " + gavr.cloudConsumption);
							minGAValidResult = gavr;
							write_Boundary=String.format("%.2f",gavr.averageDeviceBandwidth);
							////showState(gavr);
							//showDetailedState(gavr);
						}else
						{
				...
				saveSingleToFile(write_DeviceQuantity, GA_ITERATIN_START, GA_ITERATIN_STEP, GA_ITERATIN_END, write_Terminal, write_Boundary, write_GA, "genetic_result_network");
	...
	public static void saveSingleToFile(int write_DeviceQuantity, int GA_ITERATIN_START, int GA_ITERATIN_STEP, int GA_ITERATIN_END, String write_terminal, String write_boundary, ArrayList<String> ga, String filename)
	{
